"""Fixed Agent service using OpenAI Agents SDK with Gemini API.

This implementation uses the official OpenAI Agents SDK pattern
with Gemini as the LLM backend via OpenAI-compatible endpoint.
"""
import json
import logging
...
class AgentService:
    ...
    This service implements the agent pattern using OpenAI's SDK
    while connecting to Gemini's API for LLM capabilities.
    """
    ...
    def __init__(
        ...
        Args:
            llm_client: AsyncOpenAI client configured for Gemini endpoint
    ...
    def _get_tools_schema(self) -> List[Dict[str, Any]]:
        """Get OpenAI function calling schema for MCP tools.
        ...
        return [
            ...
            },
            {
                ...
                "function": {
                    "name": "list_tasks",
                    "description": "Retrieve and display the user's tasks. Use this when user wants to see, show, or list their tasks.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "status": {
                                ...
                                "description": "Filter tasks by status. Use 'all' to show everything, 'pending' for incomplete tasks, 'completed' for finished tasks.",
                                "default": "all"
                            }
                        }
                    }
            ...
            },
            {
                ...
                "function": {
                    "name": "complete_task",
                    "description": "Mark a task as complete. Use this when user says a task is done, finished, or complete.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "task_id": {
            ...
            },
            {
                ...
                "function": {
                    "name": "update_task",
                    "description": "Modify a task's title or description. Use this when user wants to change, update, or edit a task.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "task_id": {
            ...
            },
            {
                ...
                "function": {
                    "name": "delete_task",
                    "description": "Remove a task from the user's list. Use this when user wants to delete, remove, or cancel a task.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "task_id": {
    ...
    async def _execute_tool(
        ...
        Args:
            ...
            args: Tool arguments (without user_id)
